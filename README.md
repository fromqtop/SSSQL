...
| `keyScan()` | `where` の条件に使う列だけを先に読んで絞り込む（`useSheetsApi()` と併用時のみ有効。詳細は [Sheets APIモード](#sheets-apiモード)） |
...
### keyScan()：条件に使う列だけを先に絞り込む

`where` で少数の行だけを検索するとき、`keyScan()` を併用すると、まず条件に使われている列だけを読み込んで対象の行番号を特定し、該当する行だけをまとめて取得します。行数が多いシートで、ヒット件数が少ない場合に効果があります。`useSheetsApi()` と組み合わせたときだけ有効です（`SpreadsheetApp` には複数行を1回でまとめて取得する手段が無いため、`useSheetsApi()` を伴わない場合は無視されます）。

```javascript
db.useSheetsApi().keyScan().where({ status: "active", age: [">", 25] }).all();
```

**仕組みと安全性について**: Sheets API は、列の末尾に空セルが連続していると、その部分を省略して返します。`keyScan()` は、`where` の条件に「空文字が条件に一致しうるもの」（例: `{status: ""}` や `{status: ["<>", "active"]}`）が含まれる場合にこれを検出し、その場合だけシートの正確な行数を確認した上で、省略された行を空文字として補って判定します。そのため、条件の内容に関わらず結果は正しくなります。行数を確認する分だけ、わずかに追加のコストがかかります（`SpreadsheetApp` 経由。1度取得すれば以降のクエリでは再利用されます）。

**注意点**:

- `where()` を指定していない場合は無視され、通常の取得になります。
- `readCache()` と組み合わせても、`keyScan()` は毎回条件に応じて取得し直すため、キャッシュの効果はありません。
